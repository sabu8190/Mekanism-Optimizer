...
> **Summary (English):**  
> High-performance optimization mod for Mekanism: eliminates logistical pipe & cable lag, accelerates high-tick factory throughput, bypasses 2.14B transfer limits, and optimizes generators with zero tick overhead.
...
### 🚀 Key Features
- ⚡ **Logistical Transporter & Pipe Caching ($O(1)$ Lookups)**: Eliminates heavy acceptor capability checks and repetitive pathfinding lookups per tick. Reduces item and fluid pipe tick lag by up to **80% - 90%** in dense mega-factories.
- ⚡ **Universal Cable Adaptive Backoff**: Intelligently pauses redundant energy transfer calculations when destination machines or energy cubes are fully charged.
- ☀️ **Generator & Solar Activator Skylight Bypass**: Bypasses expensive world raycasting and skylight checks for Solar Generators and Solar Neutron Activators during nighttime or when buffers are full.
- 🚀 **Bypass 2.14B Auto-Eject Limit & Multi-Burst Mode**: Optional config to bypass Forge's `Integer.MAX_VALUE` (2.14 billion) throughput bottleneck per tick. Seamlessly transfers trillions of RF/FE, mB of fluids/chemicals, and tens of thousands of items in a single tick (Default: safely `false`).
- 🪵 **Dedicated Async File Logger**: Automatically logs real-time performance statistics, optimization metrics, and health reports to `.minecraft/logs/mekanism_optimizer.log` with **zero tick overhead** (non-blocking async writer).
- 🌐 **Full Localization & In-Game Config**: Includes complete English and Japanese (`ja_jp`) tooltips for in-game configuration via **Configured** or `mekanism_optimizer-common.toml`.
- 🛡️ **100% GUI Compatibility**: Ensures 100% stability and accessibility for all machines and screens, including the Greenhouse and multiblocks.
...
> **概要 (Summary 日本語訳):**  
> Mekanism 専用の次世代ハイパフォーマンス最適化 MOD。パイプ・送電網の探索負荷を最大 90% 削減し、天体倍速・超巨大ファクトリーの Tick スパイク解消、21億制限突破＆バースト自動搬出、発電機の軽量化を実現します。
...
### 🚀 主な機能と特徴 (日本語)
- ⚡ **物流パイプ＆トランスポーター探索の劇的軽量化 ($O(1)$ キャッシュ)**: 毎 Tick 行われていた搬入先マシンの Capability 照会や経路探索を $O(1)$ で高速キャッシュ化。超巨大工場におけるパイプの Tick 負荷を **最大 80% 〜 90% 削減** します。
- ⚡ **送電ケーブル適応型バックオフ**: 接続先マシンや蓄電器が満充電の場合、無駄な送電シミュレーション計算をインテリジェントに休止してサーバー負荷を抑えます。
- ☀️ **発電機＆太陽光活性化装置のスカイライト計算バイパス**: 太陽光発電機やソーラー中性子活性化装置において、夜間やバッファ満杯時に負荷の高い直射日光判定（レイキャスト）を $O(1)$ で完全バイパスします。
- 🚀 **21億（2.14B）搬出制限突破＆マルチバースト自動搬出**: Forge 標準の `Integer.MAX_VALUE`（21.4 億）による 1 Tick あたりの搬出上限を突破するオプションを搭載。1 Tick 内で数十億〜数兆超のエネルギー・液体・気体、および数万個のアイテムを一括搬出可能です（初期値: 安全な OFF）。
- 🪵 **専用非同期ログシステム (`logs/mekanism_optimizer.log`)**: 他 MOD の大量のログに埋もれることなく、Mekanism Optimizer 単独の最適化統計・稼働メトリクスを `.minecraft/logs/mekanism_optimizer.log` に非同期（Tick 負荷完全ゼロ）で自動記録します。
- 🌐 **完全日本語化コンフィグ＆ツールチップ対応**: ゲーム内設定 MOD（**Configured** 等）でマウスホバーした際、全項目の分かりやすい日本語説明ツールチップが表示されます。
